...
function cloneObject(e) {
    let t = clone(e);
    return "teleporter" === t.type && (t.id = newId()), t; // fixed teleporters
}
function targetedObject(e) {
    for (let t of types) {
        ...
        for (let i = s.length - 1; i >= 0; i--) {
            let n = s[i],
                [{ x: a, y: r }, { x: o, y: u }] = points("rotLavaPoint" === t ? { pos: n, size: { x: 0, y: 0 } } : n),
                p = { x: e.pageX, y: e.pageY };
            if ("text" === t) {
                if (pointInCircle(p, { x: a, y: r }, 5 * camScale + selectBuffer)) return n;
                continue;
            }
            if ("turret" === t) {
                if (pointInCircle(p, { x: a, y: r }, 3 * camScale + selectBuffer)) return n;
                continue;
            }
            if ("rotLavaPoint" === t) {
                if (pointInCircle(p, { x: a, y: r }, 0.5 * camScale + selectBuffer)) return n;
                continue;
            }
            if (
                pointInRect(
                    p,
                    { x: a - selectBuffer, y: r - selectBuffer },
                    { x: o + selectBuffer, y: u + selectBuffer }
                )
            )
                return n;
        }
...
canvas.addEventListener("wheel", (e) => {
    ...
    let t = 0.85 ** (e.deltaY / 125),
        s = (e.pageX - canvas.width / 2) / camScale + camX,
        i = (e.pageY - canvas.height / 2) / camScale + camY;
    (camX = (t * s - s + camX) / t), (camY = (t * i - i + camY) / t), (camScale *= t);
}),
    canvas.addEventListener("mousedown", (e) => {
        ...
        if (t) {
            ...
            let { x: i, y: n } = t.pos ?? t,
                { x: a, y: r } = t.size ?? { x: 0, y: 0 },
                o = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                u = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
            if ("rotLavaPoint" === t.type) {
                let p = t.rotLava;
                s = (e) => {
                    let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                        a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                    (p.inputs.pX.value = t.x = Math.round(s - o + i)),
                        (p.inputs.pY.value = t.y = Math.round(a - u + n));
                };
            } else if ("circularObject" === t.type)
                switch (selectMode) {
                    case "u":
                        s = (e) => {
                            let s = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            (t.inputs.radius.value = t.radius = Math.max(n + r - s, 0) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.y.value = t.pos.y = n + r - t.size.y),
                                (t.inputs.x.value = t.pos.x = i + r / 2 - t.radius);
                        };
                        break;
                    case "ur":
                        s = (e) => {
                            ...
                            (t.inputs.radius.value = t.radius =
                                Math.min(Math.max(n + r - a, 0), Math.max(s - i, 0)) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.y.value = t.pos.y = n + r - t.size.y);
                        };
                        break;
                    case "r":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX);
                            (t.inputs.radius.value = t.radius = Math.max(s - i, 0) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.y.value = t.pos.y = n + a / 2 - t.radius);
                        };
                        break;
                    case "dr":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                                a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            (t.inputs.radius.value = t.radius = Math.min(Math.max(a - n, 0), Math.max(s - i, 0)) / 2),
                                (t.size.y = t.size.x = 2 * t.radius);
                        };
                        break;
                    case "d":
                        s = (e) => {
                            let s = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            (t.inputs.radius.value = t.radius = Math.max(s - n, 0) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.x.value = t.pos.x = i + r / 2 - t.radius);
                        };
                        break;
                    case "dl":
                        s = (e) => {
                            ...
                            (t.inputs.radius.value = t.radius =
                                Math.min(Math.max(r - n, 0), Math.max(i + a - s, 0)) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.x.value = t.pos.x = i + a - t.size.x);
                        };
                        break;
                    case "l":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX);
                            (t.inputs.radius.value = t.radius = Math.max(i + a - s, 0) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.y.value = t.pos.y = n + a / 2 - t.radius),
                                (t.inputs.x.value = t.pos.x = i + a - t.size.x);
                        };
                    ...
                    case "ul":
                        s = (e) => {
                            ...
                            (t.inputs.radius.value = t.radius =
                                Math.min(Math.max(n + r - o, 0), Math.max(i + a - s, 0)) / 2),
                                (t.size.y = t.size.x = 2 * t.radius),
                                (t.inputs.x.value = t.pos.x = i + a - t.size.x),
                                (t.inputs.y.value = t.pos.y = n + r - t.size.y);
                        };
                    ...
                    case "m":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                                a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            (t.inputs.x.value = t.pos.x = s - o + i), (t.inputs.y.value = t.pos.y = a - u + n);
                        };
                }
                ...
                if ("m" === selectMode) {
                    ...
                    s = (e) => {
                        let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                            a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                        (t.inputs.x.value = t.pos.x = s - o + i),
                            (t.inputs.y.value = t.pos.y = a - u + n),
                            (t.inputs.rX.value = l.pos.x = s - o + d),
                            (t.inputs.rY.value = l.pos.y = a - u + c);
                    };
                }
                ...
                if ("m" === selectMode) {
                    ...
                    s = (e) => {
                        let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                            a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                        (v.inputs.x.value = t.pos.x = s - o + i),
                            (v.inputs.y.value = t.pos.y = a - u + n),
                            (v.inputs.rX.value = t.pos.x = s - o + y),
                            (v.inputs.rY.value = t.pos.y = a - u + x);
                    };
                }
            } else
                switch (selectMode) {
                    case "u":
                        s = (e) => {
                            let s = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            n - s + r > 0
                                ? ((t.pos.y = s), (t.size.y = n - s + r))
                                : ((t.pos.y = n + r), (t.size.y = 0)),
                    ...
                    case "ur":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                                a = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            i - s < 0 ? (t.size.x = s - i) : (t.size.x = 0),
                                n - a + r > 0
                                    ? ((t.pos.y = a), (t.size.y = n - a + r))
                                    : ((t.pos.y = n + r), (t.size.y = 0)),
                    ...
                    case "dl":
                        s = (e) => {
                            let s = Math.round((e.pageX - canvas.width / 2) / camScale + camX),
                                r = Math.round((e.pageY - canvas.height / 2) / camScale + camY);
                            i - s + a > 0
                                ? ((t.pos.x = s), (t.size.x = i - s + a))
                                : ((t.pos.x = i + a), (t.size.x = 0)),
                                n - r < 0 ? (t.size.y = r - n) : (t.size.y = 0),
                                (t.inputs.x.value = t.pos.x),
                                (t.inputs.y.value = t.pos.y),
                                (t.inputs.w.value = t.size.x),
                                (t.inputs.h.value = t.size.y);
                    ...
                    case "m":
                        if ("rotatingLava" === t.type) {
                            ...
                            s = (e) => {
                                ...
                                (t.inputs.x.value = t.pos.x = s - o + i),
                                    (t.inputs.y.value = t.pos.y = a - u + n),
                                    (t.inputs.pX.value = t.point.x = g + s - o),
                                    (t.inputs.pY.value = t.point.y = h + a - u);
                            };
                            break;
                        }
                        s = (e) => {
        ...
        if (
            ...
            let z = null;
            if ((z = map.areas.find((e) => e.name === selectedObject.targetArea))) {
                hide(currentArea.element), (currentArea = z), selectedObject && hide(selectedObject.element);
                let m = null;
                (m = z.objects.teleporter.find((e) => e.id === selectedObject.targetID))
                    ? ((camX = m.pos.x), (camY = m.pos.y), (selectedObject = m))
                    : ((selectedObject = null), (camX = z.size[0] / 2), (camY = z.size[1] / 2)),
    ...
    canvas.addEventListener("mousemove", (e) => {
        if (!lockCursor) {
            for (let t of types) {
                ...
                for (let i = s.length - 1; i >= 0; i--) {
                    let n = s[i],
                        [{ x: a, y: r }, { x: o, y: u }] = points(
                            "rotLavaPoint" === t ? { pos: n, size: { x: 0, y: 0 } } : n
                        ),
                        p = point(e);
                    if ("text" === t) {
                        if (pointInCircle(p, { x: a, y: r }, 5 * camScale + selectBuffer)) {
                    ...
                    if ("turret" === t) {
                        if (pointInCircle(p, { x: a, y: r }, 3 * camScale + selectBuffer)) {
                            (canvas.style.cursor = "grab"), (selectMode = "m");
                            return;
                        }
                        continue;
                    ...
                    if ("rotLavaPoint" === t) {
                        if (pointInCircle(p, { x: a, y: r }, 0.5 * camScale + selectBuffer)) {
                            (canvas.style.cursor = "grab"), (selectMode = "m");
                            return;
                        }
                        continue;
                    ...
                    let l = pointInRect(
                        ...
                        d = pointInRect(
                            p,
                            { x: a - selectBuffer, y: r - selectBuffer },
                            { x: o + selectBuffer, y: r + selectBuffer }
                        ),
                        c = pointInRect(
                            p,
                            { x: a - selectBuffer, y: r - selectBuffer },
                            { x: a + selectBuffer, y: u + selectBuffer }
                        ),
                        y = pointInRect(
                            p,
                            { x: a - selectBuffer, y: u - selectBuffer },
                        ...
                        v = pointInRect(p, { x: a, y: r }, { x: o, y: u });
                    if (
                        (v
                            ? ((canvas.style.cursor = "grab"), (selectMode = "m"))
                            : y
    ...
    window.addEventListener("beforeunload", (e) => (e.preventDefault(), (e.returnValue = "Have you saved your map?")));
{
    ...
    (s.value = map.settings.spawnArea),
        s.addEventListener("change", () => {
            map.areas.some((e) => e.name === s.value)
                ? (map.settings.spawnArea = s.value)
                : confirm(`Area ${s.value} not found, would you like to create it?`)
                  ? addArea(s.value)
                  : alert("You'd better make that area then.");
        });
    let i = document.createElement("input");
    (i.value = map.settings.spawnPos[0]),
        i.addEventListener("input", () => {
            map.settings.spawnPos[0] = Number(i.value);
        });
    ...
    (n.value = map.settings.spawnPos[1]),
        ...
        (map.element = createFolder("Map Properties", [
            ...
            createProperty("spawnArea", s, "text"),
            createFolder("Spawn Position", [createProperty("x", i, "number"), createProperty("y", n, "number")]),
        ])),
        map.element.classList.add("closed"),
        menu.insertBefore(map.element, areamenu),
        (map.inputs = { name: e, creator: t, spawnArea: s, spawnX: i, spawnY: n });
}
...
function pointInRect(e, t, s) {
    return e.x > t.x && e.x < s.x && e.y > t.y && e.y < s.y;
}
function pointInCircle(e, t, s) {
    return (e.x - t.x) * (e.x - t.x) + (e.y - t.y) * (e.y - t.y) <= s * s;
}
function hide(e) {
...
(copyBtn.id = "copyObject"),
    ...
    document.addEventListener("keydown", (e) => {
        if (!(e.target instanceof HTMLInputElement)) {
            if (e.key.toLowerCase() === (localStorage.getItem("hitbox") ?? "o")) {
                renderSettings.render.hitbox = !renderSettings.render.hitbox;
                return;
            }
            if (e.ctrlKey && "c" === e.key.toLowerCase() && selectedObject) {
                let t = cloneObject(selectedObject);
                if (!t) return;
                (clipboard = t), e.preventDefault();
            }
            e.ctrlKey &&
                "v" === e.key.toLowerCase() &&
                clipboard &&
