...
function getObjects(e = "obstacle") {
    return "block0" === e
        ? currentArea.objects.block.filter((e) => !e.layer)
        : "block1" === e
          ? currentArea.objects.block.filter((e) => e.layer)
          : "rotLavaPoint" === e
            ? currentArea.objects.rotatingLava.map((e) => e.point)
            : "circularObstacle" === e
              ? currentArea.objects.circularObject.filter((e) => "obstacle" === e.objectType)
              : "circularLava" === e
                ? currentArea.objects.circularObject.filter((e) => "lava" === e.objectType)
                : "circularSlime" === e
                  ? currentArea.objects.circularObject.filter((e) => "slime" === e.objectType)
                  : "circularIce" === e
                    ? currentArea.objects.circularObject.filter((e) => "ice" === e.objectType)
                    : "turretRegion" === e
                      ? currentArea.objects.turret.includes(selectedObject)
                          ? [selectedObject.region]
                          : []
                      : "movingObstacle" === e
                        ? currentArea.objects.movingObject.filter((e) => "obstacle" === e.objectType)
                        : "movingLava" === e
                          ? currentArea.objects.movingObject.filter((e) => "lava" === e.objectType)
                          : "movingSlime" === e
                            ? currentArea.objects.movingObject.filter((e) => "slime" === e.objectType)
                            : "movingIce" === e
                              ? currentArea.objects.movingObject.filter((e) => "ice" === e.objectType)
                              : e in currentArea.objects
                                ? currentArea.objects[e]
                                : [];
}
const renderSettings = {
    ...
    colors: {
        ...
        selected: "#ff0000",
        gravOutline: ["#ffff00", "#ff0000", "#0000ff", "#00ff00", "#000000"],
        gravFill: ["#ffff0008", "#ff000008", "#0000ff08", "#00ff0008", "#00000008"],
        button: "#404040",
    ...
    textures: {
        enemies: {
            ...
            bomb: [loadImage("enemies/bomb0.svg"), loadImage("enemies/bomb1.svg")],
            contractor: [loadImage("enemies/contractor0.svg"), loadImage("enemies/contractor1.svg")],
        ...
        hats: {
            ...
            solar: {
                ...
                textOffset: -5,
                size: [2.4, 2.4],
                texture: loadImage("/assets/sktextures/hats/solar.png"),
            },
        },
        powers: [
...
const randomMapNames = ["editor test", "My Map", "EPIC MAP", "{{map->name}}"],
    randomMapCreators = ["anonymous", "xXDark_L0rd_69420Xx", "Editor", "{{map->creator}}"],
    map = {
        settings: {
            name: "",
            creator: "",
...
{
    let e = ["Scroll on numbers to increment"],
        t = Math.floor(Math.random() * e.length);
    document.getElementById("tip").innerHTML = `Tip ${t + 1}: ${e[t]}`;
}
let camScale = 5;
const camSpeed = 10;
let camX = 50,
...
const menu = document.getElementById("menu"),
    ...
    objectmenu = document.getElementById("objectmenu"),
    togglemenu = document.getElementById("togglemenu"),
    resizemenu = document.getElementById("resizemenu");
let resizing = !1;
const downloadBtn = document.getElementById("download"),
    importInput = document.getElementById("import"),
    obstacleBtn = document.getElementById("createObstacle"),
    lavaBtn = document.getElementById("createLava"),
    slimeBtn = document.getElementById("createSlime"),
    iceBtn = document.getElementById("createIce"),
    ...
    togglebottommenu = document.getElementById("togglebottommenu"),
    bottommenu = document.getElementById("bottommenu"),
    areaList = document.getElementById("areaList");
function loadImage(e) {
    let t = new Image();
    return (
        (t.src = e.startsWith("http") ? e : e.startsWith("/") ? e : `../Textures/${e}`),
        (t.onerror = () => {
            console.log("ERROR AT", t.src);
        }),
        t
    );
}
...
function arrtoRGBA(e) {
    return `rgba(${e.join()})`;
}
function fillZeros(e = "0", t = 2, a = "0") {
    return a.repeat(t - e.length) + e;
}
function arrtoHex(e) {
    return `#${fillZeros(e[0].toString(16))}${fillZeros(e[1].toString(16))}${fillZeros(e[2].toString(16))}`;
}
function blend240([e, t, a], s) {
    return "rgb(" + (240 + (e - 240) * s) + "," + (240 + (t - 240) * s) + "," + (240 + (a - 240) * s) + ")";
}
let clipboard = null,
    mousePos = { x: 0, y: 0 };
function clone(e) {
