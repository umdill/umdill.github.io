...
function areaToJSON(area) {
    ...
    for (let hatReward of area.objects.hatReward) {
        objects.push(hatRewardToJSON(hatReward));
    }
...
/**
 * @param {HatReward} hatReward 
 */
function hatRewardToJSON(hatReward) {
    return `{"type":"hatReward","position":[${hatReward.pos.x + 7.5},${hatReward.pos.y + 7.5}],"reward":${JSON.stringify(hatReward.reward)}}`;
}